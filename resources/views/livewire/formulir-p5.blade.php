<div>
    ...
    <div class="row mb-2">
        ...
        <div class="col-sm-9" wire:ignore>
            <select id="tingkat" class="form-select" wire:model="tingkat" data-pharaonic="select2"
                data-component-id="{{ $this->id }}" data-placeholder="== Pilih Tingkat Kelas =="
                data-search-off="true" wire:change="changeTingkat">
    ...
    <div class="row mb-2" wire:ignore>
        ...
        <div class="col-sm-9">
            <select id="rombongan_belajar_id" class="form-select" wire:model="rombongan_belajar_id"
                data-pharaonic="select2" data-component-id="{{ $this->id }}"
                data-placeholder="== Pilih Rombongan Belajar ==" wire:change="changeRombel">
                ...
                <thead>
                    ...
                    <tr>
                        @foreach ($rencana_budaya_kerja as $rencana)
                            <th class="text-center" colspan="{{ $rencana->aspek_budaya_kerja->count() }}">
                                {{ $rencana->nama }} ({{ $rencana->pembelajaran->nama_mata_pelajaran }})</th>
                        @endforeach
                    </tr>
                    <tr>
                        @foreach ($rencana_budaya_kerja as $rencana)
                            @foreach ($rencana->aspek_budaya_kerja as $aspek)
                                <th class="text-center" style="font-style: normal;">
                                    <a href="javascript:void(0)" data-bs-toggle="tooltip" data-bs-html="true"
                                        data-bs-title="<p><strong>Dimensi:</strong> {{ $aspek->budaya_kerja->aspek }}</p>
                            ...
                            @foreach ($rencana_budaya_kerja as $rencana)
                                @foreach ($rencana->aspek_budaya_kerja as $aspek)
                                    <td class="text-center">
                                        <select id="nilai" class="form-select" wire:ignore
                                            wire:model.defer="nilai.{{ $siswa->anggota_rombel->anggota_rombel_id }}.{{ $aspek->aspek_budaya_kerja_id }}">
                                            <option value="">-</option>
                                            @foreach ($opsi_budaya_kerja as $opsi)
                                                <option
                                                    value="{{ $opsi->opsi_id }}|{{ $aspek->elemen_id ? $aspek->elemen_id : 0 }}">
                                                    {{ $opsi->nama }}</option>
                                            @endforeach
                                        </select>
                                    </td>
                        ...
                        <tr>
                            @foreach ($rencana_budaya_kerja as $rencana)
                                <td colspan="{{ $rencana->aspek_budaya_kerja->count() }}">
                                    <textarea title="Catatan proses" placeholder="Catatan proses" wire:ignore
                                        wire:model.defer="deskripsi.{{ $rencana->rencana_budaya_kerja_id }}.{{ $siswa->anggota_rombel->anggota_rombel_id }}"
