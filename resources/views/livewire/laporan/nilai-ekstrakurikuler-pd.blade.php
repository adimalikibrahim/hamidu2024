    ...
    <table class="table table-bordered">
        ...
        <tbody>
            @forelse ($data_siswa as $item)
                <tr>
                    <td rowspan="{{ $item->anggota_rombel->anggota_ekskul->count() + 1 }}" class="align-middle">
                        {{ $item->nama }}</td>
                </tr>
                @foreach ($item->anggota_rombel->anggota_ekskul as $anggota_ekskul)
                    <tr>
                        ...
                        <td>{{ $anggota_ekskul->single_nilai_ekstrakurikuler ? nilai_ekskul($anggota_ekskul->single_nilai_ekstrakurikuler->nilai) : '-' }}
                        </td>
                        <td>{{ $anggota_ekskul->single_nilai_ekstrakurikuler ? $anggota_ekskul->single_nilai_ekstrakurikuler->deskripsi_ekskul : '-' }}
                        </td>
                    </tr>
                @endforeach
            @empty
