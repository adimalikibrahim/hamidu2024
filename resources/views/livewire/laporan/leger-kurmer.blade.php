                ...
                @role(['waka', 'tu'], session('semester_id'))
                    ...
                    @if ($show && $data_pembelajaran->count())
                        <div class="row mb-2">
                            ...
                            <div class="col-sm-9">
                                <a href="{{ route('unduhan.unduh-leger-nilai-kurmer', ['rombongan_belajar_id' => $rombongan_belajar_id]) }}"
                                    class="btn btn-success">Unduh Legger</a>
                            </div>
                        </div>
                    @endif
                @endrole
                            ...
                            <thead>
                                <tr>
                                    ...
                                    @foreach ($data_pembelajaran as $pembelajaran)
                                        <th class="text-center align-middle">{{ $pembelajaran->nama_mata_pelajaran }}
                                        </th>
                                    @endforeach
                                </tr>
                            </thead>
                                        ...
                                        @foreach ($data_pembelajaran as $pembelajaran)
                                            ...
                                            if ($merdeka) {
                                                if ($pembelajaran->rombongan_belajar->jenis_rombel == 1) {
                                                    $nilai = $pembelajaran
                                                        ->nilai_akhir_kurmer()
                                                        ->where('anggota_rombel_id', $item->anggota_rombel->anggota_rombel_id)
                                                        ->first();
                                                } else {
                                                    if ($item->anggota_pilihan) {
                                                        $nilai = $pembelajaran
                                                            ->nilai_akhir_kurmer()
                                                            ->where('anggota_rombel_id', $item->anggota_pilihan->anggota_rombel_id)
                                                            ->first();
                                                    }
                                                }
                                            } else {
                                            ...
                                            @else
                                                <td class="text-center table-light">{{ $nilai ? $nilai->nilai : '-' }}
                                                </td>
                                            @endif
                                        @endforeach
                                    </tr>
                                @empty
                                    <tr>
                                        <td class="text-center"
                                            colspan="{{ $data_pembelajaran ? $data_pembelajaran->count() + 2 : 2 }}">
