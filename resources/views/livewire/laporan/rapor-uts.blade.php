                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div wire:ignore.self>
                                                <select id="rencana_penilaian_{{ $item->pembelajaran_id }}"
                                                    class="form-select form-select-sm" data-pharaonic="select2"
                                                    data-component-id="{{ $this->id }}"
                                                    wire:model="rencana_penilaian.{{ $item->pembelajaran_id }}"
                                                    data-tags="true" data-placeholder="Pilih Penilaian" multiple>
                                                    @foreach ($item->rencana_penilaian as $rencana_penilaian)
                                                        <option value="{{ $rencana_penilaian->rencana_penilaian_id }}">
                                                            {{ $rencana_penilaian->nama_penilaian }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </td>
                    ...
                    <div class="card-footer d-md-flex justify-content-between">
                        @if ($check)
                            <a target="_blank" class="btn btn-warning"
                                href="{{ url('cetak/rapor-uts/' . $rombongan_belajar_id) }}"><i class="fa fa-print"></i>
                                Cetak</a>
                        @endif
    ...
    <script>
        window.addEventListener('rencana_penilaian', event => {
            ...
            $.each(event.detail.pembelajaran_id, function(urut, pembelajaran_id) {
                ...
                $('#rencana_penilaian_' + pembelajaran_id).html(
                    '<option value="">== Pilih Penilaian Update ==</option>')
                $.each(event.detail.rencana_penilaian, function(i, item) {
                    if (item.rencana_penilaian.length) {
                        $.each(item.rencana_penilaian, function(i, rencana_penilaian) {
                            $('#rencana_penilaian_' + pembelajaran_id).append($(
                            '<option>', {
